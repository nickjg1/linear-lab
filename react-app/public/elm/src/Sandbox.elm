...
rowOfVectors model =
    let
        theVisualModel =
            getVisualModel "embed1" model.ivvlDict
        ...
        numberOfVectors =
            Dict.size theGrid.vectorObjects

        repeats =
            floor (toFloat numberOfVectors / 7)
        ...
        theVectors =
            Dict.toList theGrid.vectorObjects

        indexedInputs =
            List.indexedMap Tuple.pair theVectors
            ...
            \i ->
                E.row
                    ...
                    (let
                        ...
                        subsetValues =
                            List.map (vectorXYInput model.vectorInputs "embed1" 1) (List.map Tuple.first (List.map Tuple.second subset))
                     in
                     if i == repeats then
                        subsetValues ++ finalButton
        ...
        allRows =
            List.map rowCreator (List.range 0 repeats)
    in
    allRows
...
type alias VectorInputs =
    Dict ( String, Int, Int ) ( String, String, Bool )
...
initialModel =
    let
        ...
        widgetCommands =
            List.map (\( k, v ) -> Cmd.map (WidgetMsg k) (Tuple.second v)) (Dict.toList visualWidgets)

        viewPortCommand =
            Task.perform (\vp -> WindowResize (round vp.viewport.width) (round vp.viewport.height)) Dom.getViewport

        finalCmd =
            Cmd.batch (widgetCommands ++ [ viewPortCommand ])
    in
    ( model, finalCmd )
    ...
    case msg of
            ...
            let
                ...
                invisibilityCheck =
                    case Dict.get ( embedKey, gKey, vKey ) model.vectorInputs of
                        Nothing ->
                            Dict.insert ( embedKey, gKey, vKey ) ( "0", "0", True ) model.vectorInputs
                ...
                updatedVectorInputs =
                    Dict.update
                        ...
                        (case xy of
                            X ->
                                Maybe.map (\( _, y, p ) -> ( input, y, p ))

                            Y ->
                                Maybe.map (\( x, _, p ) -> ( x, input, p ))
                        )
                        invisibilityCheck
                ...
                finalVectorInputs =
                    Dict.update
                        ...
                        (case parseInput of
                            Nothing ->
                                Maybe.map (\( x, y, _ ) -> ( x, y, False ))

                            Just _ ->
                                Maybe.map (\( x, y, _ ) -> ( x, y, True ))
                        )
                        updatedVectorInputs

                theModel =
                    getVisualModel embedKey model.ivvlDict
                ...
                newVectorValue =
                    case parseInput of
                        Nothing ->
                            currentVisVectorValue.vector

                        Just val ->
                            case xy of
                                X ->
                                    ( val, second currentVisVectorValue.vector )

                                Y ->
                                    ( first currentVisVectorValue.vector, val )

                newVisVectorValue =
                    { currentVisVectorValue | vector = newVectorValue }

                newVectorObjects =
                    Dict.insert vKey newVisVectorValue theGrid.vectorObjects

                newGrid =
                    { theGrid | vectorObjects = newVectorObjects }

                newGridDict =
                    Dict.insert gKey newGrid theModel.grids

                newIVVLModel =
                    { theModel | grids = newGridDict }

                newIVVLModelDict =
                    Dict.insert embedKey newIVVLModel model.ivvlDict
            in
        ...
        IVVLMsg k messageForParent ->
            let
                ...
                updatedDict =
                    Dict.update k (Maybe.map (\value -> Tuple.first (IVVL.update messageForParent value))) originalDict
            in
        ...
        IVVLMoveMsg k ( x, y ) ->
            let
                ...
                updatedDict =
                    Dict.update k (Maybe.map (\value -> Tuple.first (IVVL.update (IVVL.HoldMove ( x, y )) value))) originalDict
            in
            ( { model | ivvlDict = updatedDict }, Cmd.none )
        ...
        WidgetMsg k wMsg ->
            let
                ( newWState, wCmd ) =
                    Widget.update wMsg (getWidgetModel k model.widgetDict)

                newWidgetModel =
                    Dict.update k (Maybe.map (\_ -> ( newWState, wCmd ))) model.widgetDict
            in
...
visualWidgets =
    Dict.fromList [ ( "embed1", Widget.init 500 500 "embed1" ) ]
